    ...
    <div className="min-h-[60vh] flex items-center justify-center p-6">
      <div className="bg-card border border-border rounded-2xl shadow-sm max-w-md w-full p-8 text-center">
        ...
        <div className="flex gap-3">
          <button
            onClick={() => reset()}
            className="flex-1 flex items-center justify-center gap-2 bg-brand-500 hover:bg-brand-600 text-on-brand py-2 rounded-lg text-sm font-medium"
          >
            <RotateCcw className="w-4 h-4" /> Try again
          </button>
          <button
            onClick={() => window.location.reload()}
            className="flex-1 flex items-center justify-center gap-2 border border-border text-body hover:bg-elevated py-2 rounded-lg text-sm font-medium"
          >
        ...
        {error?.digest && (
          <p className="mt-5 text-xs text-subtle font-mono">Reference: {error.digest}</p>
        )}
      </div>
    </div>
  );
