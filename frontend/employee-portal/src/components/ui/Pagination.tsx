...
export function Pagination({
  ...
  const arrow =
    "p-1.5 rounded border border-border text-body disabled:opacity-30 disabled:cursor-not-allowed hover:bg-elevated transition-colors";
  ...
  return (
    ...
    >
      <p className="text-xs text-subtle tabular-nums">
        Showing <span className="text-muted-foreground">{first}</span>–<span className="text-muted-foreground">{last}</span> of{" "}
      ...
      <div className="flex items-center gap-3">
        {onLimitChange && (
          <label className="flex items-center gap-1.5 text-xs text-subtle">
            ...
            <select
              ...
              className="bg-background border border-border rounded px-1.5 py-1 text-xs text-body focus:outline-none focus:ring-2 focus:ring-brand-500"
            >
        ...
        {pageItems(page, totalPages).map((item, i) =>
          item === "gap" ? (
            <span key={`gap-${i}`} className="px-1.5 text-xs text-subtle select-none">
              …
            </span>
          ) : (
            <button
              ...
              className={cn(
                ...
                item === page
                  ...
                  : "border-border text-muted-foreground hover:bg-elevated hover:text-body"
              )}
            >
              {item}
            </button>
