...
def test_metrics_is_plain_text_not_json_encoded(client):
    ...
    body = r.text
    # Regression guard: JSONResponse(content="...", media_type="text/plain")
    # still JSON-encodes the string, producing a leading `"` and literal
    # `\n` escapes that Prometheus cannot parse. Plain exposition format
    # must not be quoted.
    assert not body.strip().startswith('"')
    assert "\\n" not in body
    assert "dlp_scans_total" in body
