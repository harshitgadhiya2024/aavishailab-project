...
func TestAnyPolicyEnablesAIVisual(t *testing.T) {
	if anyPolicyEnablesDetector([]models.Policy{creditCardPolicy()}, "ai_visual") {
		t.Fatal("credit-card-only policy must not report ai_visual enabled")
	}
	if !anyPolicyEnablesDetector([]models.Policy{creditCardPolicy(), aiVisualPolicy()}, "ai_visual") {
		t.Fatal("expected ai_visual enabled when one policy among several has it")
	}
	if anyPolicyEnablesDetector(nil, "ai_visual") {
		t.Fatal("no policies must not report ai_visual enabled")
	}
	// ai_text is gated by the same generic helper.
	if anyPolicyEnablesDetector([]models.Policy{creditCardPolicy()}, "ai_text") {
		t.Fatal("credit-card-only policy must not report ai_text enabled")
	}
}
