...
# Blank -> the configured chat provider's own default model. Set to route
# vision classification through a specific OpenRouter model instead, e.g.
# "google/gemini-2.5-flash" (fast + cheap + strong vision) or
# "anthropic/claude-3.5-sonnet". OpenRouter exposes every model through the
# one OpenAI-compatible chat-completions surface, so switching models is a
# config change, never a code change. (If kie.ai is still on
# LLM_PROVIDER_CHAIN, a bare "claude-*" model name is routed through its
# Anthropic-native endpoint via app/claude_client.py — legacy path.)
DLP_VISION_MODEL = os.getenv("DLP_VISION_MODEL", os.getenv("DLP_VISION_MODEL_OPENROUTER", "google/gemini-2.5-flash"))
